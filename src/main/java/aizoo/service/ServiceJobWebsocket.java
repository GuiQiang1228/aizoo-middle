...
@Component
@ServerEndpoint("/serviceJobWebsocket/{jobId}")
public class ServiceJobWebsocket {
    private final static Logger logger = LoggerFactory.getLogger(ServiceJobWebsocket.class);
    ...
    public void setClient(Client client) {
        ServiceJobWebsocket.rpcClient = client;
    }

    @Autowired
    public void setJobMapper(ServiceJobDAO serviceJobDAO, UserDAO userDAO) {
        ServiceJobWebsocket.serviceJobDAO = serviceJobDAO;
    }
    ...
    static {
        ...
        if (jobUpdateThread.getState().equals(Thread.State.NEW))
            ...
            logger.error("serviceJobWebsocket jobUpdateThread started failed！ thread state= {}", jobUpdateThread.getState());
        }
    }
    ...
    public void onOpen(@PathParam("jobId") Integer jobId, Session session) {
        ...
        synchronized (lock) {
            ...
            jobList.add(jobId);
            // 2. 更新jobKeyList
            ServiceJob serviceJob = serviceJobDAO.findById(Long.parseLong(jobId.toString())).get();
    ...
    public void onMessage(@PathParam("jobId") Integer jobId, String message) throws JsonProcessingException, FileNotFoundException, UnsupportedEncodingException {
        ...
        ServiceJob serviceJob = serviceJobDAO.findById(Long.parseLong(jobId.toString())).get();
        String logPath = getLogPath(serviceJob.getEnvironment(), serviceJob.getJobKey(), logType);

        // 3. 判断当前任务是否已经结束，本状态会被传给前端
        boolean finished = !(JobStatus.needUpdate().contains(serviceJob.getJobStatus()));
        ...
        // 5. 发送数据
        Integer id = Integer.parseInt(String.valueOf(serviceJob.getId()));
    ...
    private static void sendJobData(String jobKey) throws Exception {
        ...
        // 2. 向slurm server发请求，查询job资源使用情况、运行统计信息、ServiceJob的IP和Port
        ...
        String ip = serviceJob.getIp();
        String port = serviceJob.getPort();
        ...
        // 3. 解析并组织数据
        Map<String, Object> jobData = parseJobData(serviceJob, newJobUsage, newJobSummary, ip, port);
     ...
     *
     * @param serviceJob 要解析的job对象，其environment字段已经包括了该job的分配节点信息、已运行时长、最大内存等等
     ...
     * {
     * "showData":{数据库里的serviceJob对象序列化之后的map，其中environment是去掉jobkey之后的子map},
     * "runData":{job运行信息，格式参考parseRunData方法，job还在运行时使用，和stopData不会同时不为空}
     * "stopData":{job运行完后的统计信息，job结束后使用，和runData不会同时不为空}
     * }
     * @throws JsonProcessingException
     */
    public static Map<String, Object> parseJobData(ServiceJob serviceJob, Map<String, List> jobUsage, Map<String, Object> jobSummary, String ip, String port) throws JsonProcessingException {
        String jobKey = serviceJob.getJobKey();
        logger.info("Start parseJobData,  jobKey:{}, jobUsage:{}, jobSummary:{}, ip:{}, port:{}", jobKey, jobUsage, jobSummary, ip, port);
        Map<String, Object> jobData = new HashMap<>();

        // 1. 将目标job序列化成map，填入showData
        Map<String, Object> jobMap = objectMapper.readValue(objectMapper.writeValueAsString(serviceJob), Map.class);
        // 前端需要的environment是去掉jobkey之后的map，所以后台需要预先解析数据的environment后，将里层map放入showData.environment
        String jobEnvironment = serviceJob.getEnvironment();
        ...
        jobData.put("IP",ip);
        jobData.put("Port",port);
        jobData.put("showData", jobMap);


        // 2. 根据job是否在运行，放入不同的runData和stopData
        if (stopStatus.contains(serviceJob.getJobStatus()) && jobSummary.containsKey(jobKey)) {
            // job已结束则runData为空
