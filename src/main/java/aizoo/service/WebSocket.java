...
public class WebSocket {
    ...
    private static ExperimentJobDAO experimentJobDAO;
    ...
    public void setClient(Client client) {
        WebSocket.rpcClient = client;
    }

    @Autowired
    public void setJobMapper(ExperimentJobDAO experimentJobDAO, UserDAO userDAO) {
        WebSocket.experimentJobDAO = experimentJobDAO;
    }
    ...
    static {
        ...
        if (jobUpdateThread.getState().equals(Thread.State.NEW))
            ...
            logger.error("websocket jobUpdateThread started failed！ thread state= {}", jobUpdateThread.getState());
        }
    }
    ...
    public void onOpen(@PathParam("jobId") Integer jobId, Session session) {
        ...
        synchronized (lock) {
            ...
            jobList.add(jobId);
            // 2. 更新jobKeyList
            ExperimentJob experimentJob = experimentJobDAO.findById(Long.parseLong(jobId.toString())).get();
    ...
    public void onMessage(@PathParam("jobId") Integer jobId, String message) throws JsonProcessingException, FileNotFoundException, UnsupportedEncodingException {
        ...
        ExperimentJob experimentJob = experimentJobDAO.findById(Long.parseLong(jobId.toString())).get();
        String logPath = getLogPath(experimentJob.getEnvironment(), experimentJob.getJobKey(), logType);


        // 3. 判断当前任务是否已经结束，本状态会被传给前端
        boolean finished = !(JobStatus.needUpdate().contains(experimentJob.getJobStatus()));
        ...
        // 5. 发送数据
        Integer id = Integer.parseInt(String.valueOf(experimentJob.getId()));
    ...
    private static void sendJobData(String jobKey) throws Exception {
        ...
        // 2. 向slurm server发请求，查询job资源使用情况和运行统计信息
        ...
        String rootPathMapJsonStr = objectMapper.writeValueAsString(rootPathMap);
        String jobLossAndMetric = rpcClient.getJobLossAndMetric(jobKey, slurmAccount, rootPathMapJsonStr);
        Map<String, List> newJobLossAndMetric = objectMapper.readValue(jobLossAndMetric, new TypeReference<Map<String, List>>() {
        });
        // 3. 解析并组织数据
        Map<String, Object> jobData = parseJobData(experimentJob, newJobUsage, newJobSummary, newJobLossAndMetric);
     ...
     *
     * @param experimentJob 要解析的job对象，其environment字段已经包括了该job的分配节点信息、已运行时长、最大内存等等
     ...
     * {
     * "showData":{数据库里的experimentJob对象序列化之后的map，其中environment是去掉jobkey之后的子map},
     * "runData":{job运行信息，格式参考parseRunData方法，job还在运行时使用，和stopData不会同时不为空}
     * "stopData":{job运行完后的统计信息，job结束后使用，和runData不会同时不为空}
     * }
     * @throws JsonProcessingException
     */
    public static Map<String, Object> parseJobData(ExperimentJob experimentJob, Map<String, List> jobUsage, Map<String, Object> jobSummary, Map<String, List> jobLossAndMetric) throws JsonProcessingException {
        String jobKey = experimentJob.getJobKey();
        logger.info("Start parseJobData,  jobKey:{}, jobUsage:{}, jobSummary:{}", jobKey, jobUsage, jobSummary);
        Map<String, Object> jobData = new HashMap<>();

        // 1. 将目标job序列化成map，填入showData
        Map<String, Object> jobMap = objectMapper.readValue(objectMapper.writeValueAsString(experimentJob), Map.class);
        // 前端需要的environment是去掉jobkey之后的map，所以后台需要预先解析数据的environment后，将里层map放入showData.environment
        String jobEnvironment = experimentJob.getEnvironment();
        ...
        Map<String, List> LossAndMetricMap = new HashMap<>();
        for (String key: jobLossAndMetric.keySet()) {
            if (jobLossAndMetric.get(key).size()>1000) { // 若列表长度>100只取后100个数据
                LossAndMetricMap.put(key, jobLossAndMetric.get(key).subList(jobLossAndMetric.get(key).size()-1000,jobLossAndMetric.get(key).size()));
            } else { // 若列表长度<=100则全部保留
                LossAndMetricMap.put(key, jobLossAndMetric.get(key));
            }
        }
        jobMap.put("LossAndMetric", LossAndMetricMap);
        jobData.put("showData", jobMap);


        // 2. 根据job是否在运行，放入不同的runData和stopData
        if (stopStatus.contains(experimentJob.getJobStatus()) && jobSummary.containsKey(jobKey)) {
            // job已结束则runData为空
