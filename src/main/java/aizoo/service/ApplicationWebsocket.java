...
@Component
@ServerEndpoint("/ApplicationWebsocket/{jobId}")
public class ApplicationWebsocket {
    private final static Logger logger = LoggerFactory.getLogger(ApplicationWebsocket.class);
    ...
    public void setClient(Client client) {
        ApplicationWebsocket.rpcClient = client;
    }

    @Autowired
    public void setJobMapper(ApplicationDAO applicationDAO, UserDAO userDAO) {
        ApplicationWebsocket.applicationDAO = applicationDAO;
    }
    ...
    static {
        ...
        if (jobUpdateThread.getState().equals(Thread.State.NEW))
            ...
            logger.error("applicationWebsocket jobUpdateThread started failed！ thread state= {}", jobUpdateThread.getState());
        }
    }
    ...
    public void onOpen(@PathParam("jobId") Integer jobId, Session session) {
        ...
        synchronized (lock) {
            ...
            jobList.add(jobId);
            // 2. 更新jobKeyList
            Application application = applicationDAO.findById(Long.parseLong(jobId.toString())).get();
    ...
    public void onMessage(@PathParam("jobId") Integer jobId, String message) throws JsonProcessingException, FileNotFoundException, UnsupportedEncodingException {
        ...
        Application application = applicationDAO.findById(Long.parseLong(jobId.toString())).get();
        String logPath = getLogPath(application.getEnvironment(), application.getJobKey(), logType);

        // 3. 判断当前任务是否已经结束，本状态会被传给前端
        boolean finished = !(JobStatus.needUpdate().contains(application.getJobStatus()));
        ...
        // 5. 发送数据
        Integer id = Integer.parseInt(String.valueOf(application.getId()));
    ...
    private static void sendJobData(String jobKey) throws Exception {
        ...
        Application application = applicationDAO.findByJobKey(jobKey);

        // 2. 向slurm server发请求，查询job资源使用情况、运行统计信息、Application
        ...
        // 3. 解析并组织数据
        Map<String, Object> jobData = parseJobData(application, newJobUsage, newJobSummary);
     ...
     *
     * @param application 要解析的job对象，其environment字段已经包括了该job的分配节点信息、已运行时长、最大内存等等
     ...
     * @throws JsonProcessingException
     */
    public static Map<String, Object> parseJobData(Application application, Map<String, List> jobUsage, Map<String, Object> jobSummary) throws JsonProcessingException {
        String jobKey = application.getJobKey();
        logger.info("Start parseJobData,  jobKey:{}, jobUsage:{}, jobSummary:{}", jobKey, jobUsage, jobSummary);
        Map<String, Object> jobData = new HashMap<>();

        // 1. 将目标job序列化成map，填入showData
        Map<String, Object> jobMap = objectMapper.readValue(objectMapper.writeValueAsString(application), Map.class);
        // 前端需要的environment是去掉jobkey之后的map，所以后台需要预先解析数据的environment后，将里层map放入showData.environment
        String jobEnvironment = application.getEnvironment();
        ...
        if (stopStatus.contains(application.getJobStatus()) && jobSummary.containsKey(jobKey)) {
            // job已结束则runData为空
